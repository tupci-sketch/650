...
/* --------------------------------------------------------- draft: pool -- */
/* Each round DEALS THREE candidates, drawn with equal chance from the eligible
   pool. You pick one of the three, then choose which open seat they take. */
G.UI.renderPool = function () {
  ...
  if (!choices || !choices.length) {
    pool.innerHTML = '<p class="assign-note" style="color:var(--ink-soft)">Tap <b>Deal three candidates</b>. Three names are drawn at random \u2014 <b>you</b> pick one, then choose which open seat they fill. Re-draw the three if you have re-draws left.</p>';
    return;
  }
  ...
  choices.forEach(function (p) {
    ...
    card.className = "deal-card choice" + (despot ? " is-despot" : "") + (chosen ? " chosen" : "");
...
G.UI.renderAbout = function () {
  $("aboutBody").innerHTML =
    ...
    '<p>No overall majority? If another party — or two — can carry you over the line, you can strike a <b>coalition</b> (the closer your politics, the steadier the deal — from natural through workable and strained to unlikely), or try to govern as a <b>minority</b> on a knife-edge. Decline, and you cross to the <b>Opposition</b>: a parallel term where you steer <b>Public support</b>, <b>Momentum</b> against the government and your own <b>Party unity</b>, fight by-elections, and try to force an early election and sweep into power — or be deposed by your own side first.</p>' +
