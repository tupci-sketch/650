...
var SCREENS = ["screen-menu", "screen-draft", "screen-watch", "screen-result", "screen-about", "screen-explore", "screen-govern", "screen-legacy", "screen-policy", "screen-leaderboard", "screen-account", "screen-chat", "screen-admin", "screen-live"];
...
/* --------------------------------------------------------- draft: pool -- */
/* The wheel DEALS a random minister (chance sits on a scale — see engine).
   You don't pick the person, but you DO choose which open seat they take:
   this card reveals who you drew; the cabinet below shows where they fit. */
G.UI.renderPool = function () {
  var pool = $("pool"); pool.innerHTML = "";
  var d = G.state.lastDeal;
  if (!d) {
    pool.innerHTML = '<p class="assign-note" style="color:var(--ink-soft)">Tap <b>Deal a minister</b>. Fortune hands you someone at random \u2014 then <b>you</b> choose which open seat they fill. You can re-draw if you have re-draws left.</p>';
    return;
  }
  var p = d.politician, hard = G.state.hard, placed = !!d.placed;
  ...
  var stats = hard ? '' :
    ...
    '</div>';
  var fitLine = placed
    ? '<div class="dc-assign">\u2713 appointed \u2014 deal the next minister</div>'
    : (d.fitSeatNames && d.fitSeatNames.length
        ? '<div class="dc-assign">Fits: ' + G.UI._esc(d.fitSeatNames.slice(0,4).join(", ")) + (d.fitSeatNames.length>4?"\u2026":"") + ' \u00b7 <b>tap a seat below to appoint</b></div>'
        : '<div class="dc-assign">Fits no open seat \u2014 place them anywhere as a stretch (tap a seat below)</div>');
  var main = '<div class="dc-main">' +
    ...
    fitLine +
    (d.despot ? '<div class="dc-despot">\u26a0 a despot \u2014 appointing them is a national scandal</div>' : '') +
    stats +
    '</div>';
...
G.UI.refreshControls = function () {
  ...
  var pend = G.state.pendingPick || null;
  var spin = $("spinBtn");
  spin.disabled = complete || !!pend;
  spin.textContent = complete ? "Cabinet complete"
                   : pend ? "\u2191 Appoint them to a seat"
                   : (G.state.lastDeal ? "Deal the next minister" : "Deal a minister");

  /* the first skip slot is now the RE-DRAW control; the second is retired. */
  var rd = $("skipEraBtn"), redos = G.redosLeft ? G.redosLeft() : 0;
  if (rd) {
    if (pend && redos > 0) { rd.style.display = ""; rd.disabled = false; rd.textContent = "\u21bb Re-draw (" + redos + " left)"; }
...
G.UI.renderResult = function (res) {
  ...
  if (res.tier.govt) {
    ...
    banner.textContent = res.seats <= 0 ? "Wiped out — no seats in the House" : "A minor party on the back benches";
...
(function () {
  ...
  G.UI.renderChat = function (messages) {
    ...
    feed.innerHTML = messages.map(function (m) {
      ...
      return '<div class="chat-msg"><span class="chat-meta"><b>' + esc(m.display) + '</b>' + badge + ' <span class="chat-time">' + t + '</span>' + del + '</span><span class="chat-text">' + esc(m.text) + '</span></div>';
    }).join("");
