...
G.poolFor = function (opts) {
  ...
  if (mode === "parl2024") return G.POLITICIANS.filter(function (p) { return p.mp2024; });
...
/* The single call the UI makes to start a round.
   Deals THREE candidates, drawn UNIFORMLY AT RANDOM from the eligible, still-
   undrafted pool — every eligible figure has an equal chance of appearing (the
   pool is already scoped to the chosen game mode). Despots stay wildcard-only.
   The player then picks one of the three (G.chooseFromDeal), and chooses the
   seat (G.assignTo). Returns the candidate array, or null when the draft is done. */
G.deal = function () {
  ...
  st.choices = sampleN(und, Math.min(3, und.length));   // uniform, distinct — equal chance for every figure
  st.pendingPick = null;
  st.spin = null;
  st.lastDeal = null;
  st.spinsTaken++;
