...
G.LB.submit = function (raw, cb) {
  ...
  var payload = { game: "650", kind: "submit", name: e.name, seats: e.seats, legacy: e.legacy, govt: e.govt,
                  mode: e.mode, difficulty: e.difficulty, cabinetSize: e.cabinetSize, cabinet: e.cabinet, breakdown: e.breakdown };
  if (G.LB.URL) {
    G.LB._post(payload, function (d) {
      if (d && d.top) { cb(d.top, d.ok !== false, (d.ok === false) ? (d.error || "rejected") : null); }
      else { var top = G.LB._mergeBest(G.LB._load(), e); G.LB._save(top); cb(top, false, "offline"); }
    });
  } else { var top = G.LB._mergeBest(G.LB._load(), e); G.LB._save(top); cb(top, false); }
};
...
G.LB.recordLocalRun = function (raw) {
  ...
  G.LB._post({ game: "650", kind: "log", name: e.name, seats: e.seats, legacy: e.legacy, govt: e.govt,
