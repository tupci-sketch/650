...
(function () {
  ...
  function boot() {
    ...
    updateEligibility();
    if (G.NET) {
      G.NET.onAuth = function (me) { G.UI.applyAuth(me); };
      G.NET.resume();
  ...
  function wireSetup() {
    ...
    if (rp) rp.onclick = function () {
      ...
      buildEraToggles(true); buildDynastyChips(); updateHint(); updateEligibility();
  ...
  G.ctrl = {
    ...
    redraw:   function () { if (G.state.pendingPick && G.redosLeft && G.redosLeft() > 0) spinFlourish(function () { G.redraw(); }); },
  ...
  function wireDraft() {
    ...
    sel("skipEraBtn").onclick = function () { G.ctrl.redraw(); };   // re-draw the dealt minister
  ...
  function revealRate() {
    // total seconds to call all 650 — slower, more like a real results night
    var secs = choice.speed === "slow" ? 48 : choice.speed === "fast" ? 6 : 20;
  ...
  function wireLeaderboard() {
    ...
    sel("lbName").addEventListener("change", function () { G.LB.setName(this.value); });
    sel("resultLbBtn").onclick = submitToLeaderboard;
    sel("legacyLbBtn").onclick = submitToLeaderboard;
  }
  function openLeaderboard() {
    var n = sel("lbName"); if (n) n.value = G.LB.getName();
  ...
  function currentEntry() {
    ...
    var nameField = sel("lbName"); var name = (nameField && nameField.value) || G.LB.getName() || "";
  ...
  function submitToLeaderboard() {
    ...
    if (!e.name) {
      var nm = "";
      try { nm = (window.prompt && window.prompt("Name for the leaderboard:", "")) || ""; } catch (x) {}
      nm = G.LB.normName(nm); if (!nm) { openLeaderboard(); return; }
      e.name = nm; G.LB.setName(nm);
    }
    ...
    G.LB.submit(e, function (top, communal, err) {
      ...
      else if (err === "name taken") setLbBtns(false, "\u2605 Post to leaderboard"); // change name & retry
  ...
  function wirePlatform() {
    ...
    sel("apLoad").onclick = function () {
      ...
      var want2024 = !!sel("apMode2024").checked;
      var matches = (G.POLITICIANS || []).filter(function (p) { return p.name.toLowerCase() === name.toLowerCase(); });
      if (!matches.length) { if (m) m.textContent = "No built-in figure called \u201c" + name + "\u201d — fill the fields to add them as new."; return; }
      // prefer the record matching the checkbox; else the 2024 seed; else the first
      var fig = matches.filter(function (p) { return want2024 ? p.scope === "p24" : p.scope !== "p24"; })[0]
             || matches.filter(function (p) { return p.scope === "p24"; })[0] || matches[0];
      sel("apName").value = fig.name;
      sel("apParty").value = fig.party || "";
      sel("apFits").value = (fig.fits || []).join(",");
      var s = fig.stats || {};
      sel("apStats").value = [s.appeal, s.experience, s.oratory, s.statecraft, s.partyMgmt].map(function (x) { return x == null ? "" : x; }).join(",");
      if (sel("apNote")) sel("apNote").value = fig.note || "";
      sel("apMode2024").checked = (fig.scope === "p24");
    ...
    sel("apSave").onclick = function () {
      ...
      var name = (sel("apName").value || "").trim();
      var m = sel("apMsg");
      if (!name) { if (m) m.textContent = "A name is required."; return; }
      var is2024 = !!sel("apMode2024").checked;
      var scope = is2024 ? "p24" : "uk";
      // reuse the era of the existing record being overridden, if any
      var existing = (G.POLITICIANS || []).filter(function (p) { return p.name.toLowerCase() === name.toLowerCase() && p.scope === scope; })[0];
      var era = existing ? existing.era : (is2024 ? "e24" : "e7");
      var st = (sel("apStats").value || "").split(",").map(function (x) { return parseInt(x, 10) || 0; });
      var pol = {
        name: name, party: (sel("apParty").value || "").trim(),
        era: era, scope: scope,
        fits: (sel("apFits").value || "").split(",").map(function (s) { return s.trim(); }).filter(Boolean),
        stats: { appeal: st[0], experience: st[1], oratory: st[2], statecraft: st[3], partyMgmt: st[4] },
        note: (sel("apNote") && sel("apNote").value || "").trim(),
        mode: is2024 ? "parl2024" : ""
      };
      ...
      G.NET.adminAddPol(pol).then(function (d) {
        if (m) m.textContent = (d && d.ok)
          ? ((existing ? "Overrode " : "Added ") + name + ". Saved to the sheet — live now, and for everyone at next load.")
