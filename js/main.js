...
(function () {
  ...
  function wirePlatform() {
    ...
    sel("apSave").onclick = function () {
      ...
      var pol = { name: (sel("apName").value || "").trim(), party: (sel("apParty").value || "").trim(),
                  ...
                  mode: sel("apMode2024").checked ? "parl2024" : "" };
      var m = sel("apMsg"); if (m) m.textContent = "Saving\u2026";
      G.NET.adminAddPol(pol).then(function (d) {
        if (m) m.textContent = (d && d.ok) ? "Saved. Roster updated." : ("Failed: " + ((d && d.error) || "offline"));
