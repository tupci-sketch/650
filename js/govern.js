...
G.startTerm = function (res, opts) {
  ...
  if (opts.coalition) {
    ...
    /* concessions won from you at the negotiating table (if any) */
    var conc = opts.concessions || null;
    if (conc && conc.unityPen) unity = G._clampM(unity - conc.unityPen);
    ...
    if (G.buildCoalitionCabinet && res.opposition && G.state && G.state.cabinet) {
      var built = G.buildCoalitionCabinet(co, res.opposition, G.state.cabinet,
                    { sysKey: sysKey, playerSeats: res.seats,
                      forcePosts: conc ? conc.posts : null, capBonus: conc ? conc.capBonus : 0 });
  ...
  /* a pledge conceded at the negotiating table is struck from the manifesto */
