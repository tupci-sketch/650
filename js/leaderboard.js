...
/* ---- run identity ---------------------------------------------------------
   Every election is a unique TURN: the engine mints a runId at hold(). The
   run's record key (runSig) = runId + cabinet + mode + difficulty + size, so
   ONE election = ONE record — seats always, legacy filled in (in place) once
   that term is governed. Re-running the same cabinet mints a new runId and
   therefore a brand-new record. */
G.LB.runSig = function (e) {
...
/* ---- shareable run codes --------------------------------------------------
   ...
   whose fingerprint already scored can never count again (see the backend). */
G.LB.encodeRun = function (o) {
  try {
    var b = btoa(unescape(encodeURIComponent(JSON.stringify(o))));
    return b.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");   // URL-safe
  } catch (e) { return ""; }
};
G.LB.decodeRun = function (s) {
  try {
...
/* the run's identity for anti-replay: deterministic over settings + cabinet
   (by position) + seed + model. Any exact replay yields the same fingerprint. */
G.LB.runFingerprint = function (o) {
  if (!o) return "";
  var cab = (o.cab || []).map(function (c) { return c.k + ":" + c.n; }).sort().join(",");
  var s = [o.m, o.l || "", o.d, o.z, o.sc || "", o.sy || "", cab, (o.s >>> 0), o.mv || ""].join("|");
  return (G.hash32 ? G.hash32(s) >>> 0 : s).toString(36);
};

/* ---- anti double-submit: the exact same posted STATE of a run ------------ */
G.LB.signature = function (e) {
  return [G.LB.runSig(e), G.LB.normKey(e.name), e.seats,
          (e.legacy == null ? "-" : e.legacy), e.govt ? 1 : 0].join("|");
};
...
G.LB.submit = function (raw, cb, opts) {
  ...
  /* the communal board is for REGISTERED players only: the server takes the
     identity from the session token and rejects anything without one. */
...
/* merge a fresh snapshot of a run over its existing record:
   • a brand-new election (different seats) resets legacy until governed;
   • the same election re-recorded keeps any legacy it already earned;
   • a governed update writes its legacy in place. */
G.LB._mergeRun = function (existing, incoming) {
  var out = incoming;
  if (incoming.legacy == null && existing && existing.seats === incoming.seats && existing.legacy != null) {
    out.legacy = existing.legacy;
  }
  return out;
};
G.LB.recordLocalRun = function (raw) {
  var e = G.LB.cleanEntry(raw); e.name = e.name || G.LB.getName() || "You";
  var sig = G.LB.runSig(e);
  var runs = G.LB._loadRuns(), placed = false;
  for (var i = 0; i < runs.length; i++) {
    if (G.LB.runSig(runs[i]) === sig) { runs[i] = G.LB._mergeRun(runs[i], e); e = runs[i]; placed = true; break; }
  }
  if (!placed) runs.push(e);
  runs.sort(function (a, b) { return b.ts - a.ts; });   // newest first
  G.LB._saveRuns(runs);
  /* fire-and-forget: log to the owner's sheet too (run history, signed-in
